...
# NOVAS FUNÇÕES PARA HISTÓRICO DE MOVIMENTAÇÕES

def filtrar_movimentacoes_por_id(produto_id):
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute('''
        SELECT m.data, p.nome, m.tipo, m.quantidade, m.observacao
...
def filtrar_movimentacoes(nome_ou_id, tipo):
    conn = conectar()
    cursor = conn.cursor()
    ...
    if nome_ou_id:
        query += ' AND (p.nome LIKE ? OR m.produto_id = ?)'
        try:
            produto_id = int(nome_ou_id)
        except:
            produto_id = -1
        params.extend([f"%{nome_ou_id}%", produto_id])
